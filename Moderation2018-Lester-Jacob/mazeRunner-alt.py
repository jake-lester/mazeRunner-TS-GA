...
#24bit Version 1.0  -- March 18 2018
...
class MazeRunner_Run():
	...
	Made for organisms of 24 bitstring length"""
	def __init__(self):
		print()
		print("New Run! 24 bit")
	...
	def fitScore(self, seq, ID):
		...
		Assign scores based on cummulitive path cost... top scores for more
		unique visited nodes and for lower cumulative costs"""
		self.orgLocation = "a"
		self.visited = ["a"]
		self.costs = 0
	...
	def move(self, start, direction):
		"""helper fitness function -- moves organism from one node to anothher"""
	...
	def goalCheck(self):
		"""helper fitness function -- stops fitness function if organism visits every node """
		if self.visited == 11:
			#self.costs -= 500
			return True
		else:
	...
	def select(self):
		...
		For the purpose of this study, TOPX is an integer variable. sort population
		by ascending scores"""
		self.population.sort(key=itemgetter(1)) #sort by self.population[X][1])
	...
	def newGeneration(self):
		...
		for pair in range(15):
			parent3 = self.population[2*pair][0]
			parent4 = self.population[(2*pair)+1][0]
			children = self.crossover(parent1, parent2)
			for child in range(2):
				self.population.append([children[child]])
		self.epochsRemain -= 1
	...
	def crossover(self, parent1, parent2):
		"""create new population by breeding pairs of organisms from old population"""
		newOrganism1 = ''
		for gene in range(12):
...
start = MazeRunner_Run()#epochs, popSize)